...
namespace switched_model {
    SwitchedModelPreComputation::SwitchedModelPreComputation(const SwingTrajectoryPlanner &swingTrajectoryPlanner,
        ...
        intermediateLinearOutputAdInterface_ = std::make_unique<ocs2::CppAdInterface>(
            intermediateDiffFunc, STATE_DIM + INPUT_DIM, intermediateLibName,
            settings.autodiffLibraryFolder_);
        ...
        auto prejumpDiffFunc = [&](const ad_vector_t &x, ad_vector_t &y) {
            prejumpLinearOutputs(adComModel, adKinematicModel, x, y);
        };
        prejumpLinearOutputAdInterface_ = std::make_unique<ocs2::CppAdInterface>(
            prejumpDiffFunc, STATE_DIM, prejumpLibName, settings.autodiffLibraryFolder_);
    ...
    void SwitchedModelPreComputation::request(ocs2::RequestSet request, scalar_t t, const vector_t &x,
                                              const vector_t &u) {
        updateFeetPhases(t);
    ...
    void SwitchedModelPreComputation::updateMotionReference(scalar_t t) {
        ...
        if (uRef.head<3 * NUM_CONTACT_POINTS>().isZero()) {
            uRef.head<3 * NUM_CONTACT_POINTS>() = weightCompensatingInputs(robotMass_, contactFlags_, eulerAngles).head<
                3 * NUM_CONTACT_POINTS>();
        }
    ...
    void SwitchedModelPreComputation::intermediateLinearOutputs(const ad_com_model_t &adComModel,
        ...
        const auto o_footVelocitiesAsArray = adKinematicsModel.feetVelocitiesInOriginFrame(
    ...
    void SwitchedModelPreComputation::updateIntermediateLinearOutputs(scalar_t t, const vector_t &tapedStateInput) {
        ...
        // Read joint Torques
        jointTorques_ = intermediateLinearOutputs.segment<JOINT_COORDINATE_SIZE>(
            3 * NUM_CONTACT_POINTS + 3 * NUM_CONTACT_POINTS);

        // Read collision bodies (excluding the feet)
        for (int collisionIndex = 0; collisionIndex < collisionRadii_.size(); ++collisionIndex) {
            const int indexInOutputs = JOINT_COORDINATE_SIZE + 3 * (
                                           NUM_CONTACT_POINTS + NUM_CONTACT_POINTS + collisionIndex);
    ...
    void SwitchedModelPreComputation::updateIntermediateLinearOutputDerivatives(
        scalar_t t, const vector_t &tapedStateInput) {
        // Evaluate autodiff
        const auto intermediateLinearOutputDerivatives = intermediateLinearOutputAdInterface_->getJacobian(
            tapedStateInput);

        for (int leg = 0; leg < NUM_CONTACT_POINTS; ++leg) {
            const int indexInOutputs = 3 * leg;
            feetPositionInOriginFrameStateDerivative_[leg] = intermediateLinearOutputDerivatives.block<3, STATE_DIM>(
                indexInOutputs, 0);
            const auto &footPhase = getFootPhase(leg);
            collisionSpheresActive_[leg] = !footPhase.contactFlag();
            if (collisionSpheresActive_[leg]) {
        ...
        for (int leg = 0; leg < NUM_CONTACT_POINTS; ++leg) {
            ...
            feetVelocitiesInOriginFrameDerivative_[leg].dfdx = intermediateLinearOutputDerivatives.block<3, STATE_DIM>(
                indexInOutputs, 0);
            feetVelocitiesInOriginFrameDerivative_[leg].dfdu = intermediateLinearOutputDerivatives.block<3, INPUT_DIM>(
                indexInOutputs, STATE_DIM);
        }

        // Read joint Torques
        jointTorquesDerivative_.f = jointTorques_;
        jointTorquesDerivative_.dfdx =
                intermediateLinearOutputDerivatives.block<JOINT_COORDINATE_SIZE, STATE_DIM>(
                    3 * NUM_CONTACT_POINTS + 3 * NUM_CONTACT_POINTS, 0);
    ...
    void SwitchedModelPreComputation::prejumpLinearOutputs(const ad_com_model_t &adComModel,
                                                           const ad_kinematic_model_t &adKinematicsModel,
